...
<html lang="${langCode}" dir="${empty ltr ? 'ltr' : ltr ? 'ltr' : 'rtl'}">
    <head>
        ...
        <title>
            <fmt:message key="GENERAL_APP_NAME" /> v<fmt:message key="GENERAL_VERSION" /> - <fmt:message key="ERR_ERROR" />
        </title>
        ...
        <div id="mainPage">
            ...
            <div id="PageContent">
                <div class="contentContainer">
                    <div class="PageName">
                        <i class="glyphicon glyphicon-exclamation-sign" style="color:red;"></i> <fmt:message key="ERR_ERROR" />
                    </div>
                    <div style="padding: 0px 15px;">
                        <div class="alert alert-danger" role="alert">
                            <%
                                if (ExceptionUtility.hasCause(exception, TrustException.class)) {
                                    com.dai.trust.common.MessageProvider msgProvider = new com.dai.trust.common.MessageProvider(session.getAttribute(LanguageService.LANG_SESSION).toString());
                                    out.println(msgProvider.getMessage(ExceptionUtility.getCause(exception, TrustException.class).getMessage()));
                                } else {
                            %>
                            <fmt:message key="ERR_UNEXPECTED_ERROR" />
                            <%
                                }
                            %>
                        </div>
